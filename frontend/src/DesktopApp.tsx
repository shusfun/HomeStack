...
export function DesktopApp() {
  ...
  return <div className="desktop-shell">
    ...
    <main className="desktop-content">
      <Routes>
        <Route path="/" element={session.logged_in ? <DeviceList devices={devices} tailnet={tailnet} busy={busy} setBusy={setBusy} setError={setError} refresh={refresh} /> : <LoginPanel onLogin={refresh} />} />
        <Route path="/enroll" element={session.logged_in ? <EnrollmentPage /> : <LoginPanel onLogin={refresh} />} />
        ...
        <Route path="*" element={<DeviceList devices={devices} tailnet={tailnet} busy={busy} setBusy={setBusy} setError={setError} refresh={refresh} />} />
      </Routes>
...
function DeviceList({ devices, tailnet, busy, setBusy, setError, refresh }: { devices: DeviceView[]; tailnet: TailnetStatus | null; busy: boolean; setBusy: (value: boolean) => void; setError: (value: string) => void; refresh: () => Promise<void> }) {
  async function connect() { setBusy(true); setError(""); try { await Call.ByName(`${service}.ConnectTailnet`); await refresh(); } catch (reason) { setError(errorMessage(reason)); } finally { setBusy(false); } }
  ...
  return <section className="device-page"><div className="summary-line"><div><h1>设备</h1><p>{tailnet?.online ? `${tailnet.connection} · ${tailnet.tailnet_ip || "Tailnet"}` : "Tailscale 未连接"}</p></div>{!tailnet?.online && <button className="primary-button" disabled={busy} onClick={() => void connect()}><Network size={16} />连接 Tailnet</button>}</div>{devices.length === 0 ? <EmptyState icon={<Unplug size={24} />} label="尚未配对设备" /> : <div className="device-list">{devices.map((device) => <article className="device-row" key={device.id}><span className="device-icon"><HardDrive size={19} /></span><div className="device-copy"><strong>{device.name}</strong><small>{device.status?.tailnet_ip || device.agent_url}</small></div><StatusPill label={device.status?.online ? "在线" : "离线"} tone={device.status?.online ? "good" : "muted"} /><StatusPill label={device.status?.connection || "未连接"} tone={connectionTone(device.status?.connection)} /><button className="icon-button" onClick={() => void open(device.id)} title="用浏览器打开" aria-label={`打开 ${device.name}`}><ExternalLink size={17} /></button></article>)}</div>}</section>;
}

function EnrollmentPage() {
  const navigate = useNavigate();
  async function create(policy: EnrollmentPolicy) { return await Call.ByName(`${service}.CreateEnrollment`, policy) as { command: string; expires_at: string }; }
  return <section className="compact-page"><button className="back-button" onClick={() => navigate(-1)}><ArrowLeft size={16} />返回</button><h1>配对 Linux 设备</h1><EnrollmentForm create={create} /></section>;
}
