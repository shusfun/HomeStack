...
function Files() {
  ...
  return <AgentPage title="文件" action={<button className="icon-button" onClick={() => void load(path)} title="刷新" aria-label="刷新"><RefreshCw size={16} /></button>}><div className="file-toolbar"><div className="pathbar"><button className="icon-button" disabled={path === "/" || results !== null} onClick={() => void load(parent)} title="上一级"><ChevronLeft size={17} /></button><code>{results ? `搜索：${query}` : path}</code></div><div className="file-search"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void search(); }} aria-label="搜索文件" /><button className="icon-button" onClick={() => void search()} title="搜索" aria-label="搜索"><Search size={17} /></button></div></div><div className="file-table">{items.map((item) => <div className="file-row" key={`${item.type}-${itemPath(item)}`}><button onClick={() => activate(item)}><span>{item.type === "directory" ? <Folder size={17} /> : <File size={17} />}<strong>{item.name}</strong></span></button><span>{item.type === "directory" ? "文件夹" : formatBytes(item.size)}</span><span>{formatTime(item.modified)}</span>{item.type !== "directory" && <div className="file-actions"><button className="icon-button" onClick={() => setPreview(itemPath(item))} title="预览" aria-label={`预览 ${item.name}`}><Eye size={16} /></button><a className="icon-button" href={`/api/files/raw?files=${encodeURIComponent(itemPath(item))}&download=1`} title="下载" aria-label={`下载 ${item.name}`}><Download size={16} /></a></div>}</div>)}</div>{items.length === 0 && !error && <EmptyState icon={<FolderOpen size={24} />} label={results ? "没有匹配文件" : "目录为空"} />}{!resource && !error && <CenteredLoader label="读取文件" compact />}<Feedback error={error} />{preview && <FilePreview path={preview} onClose={() => setPreview("")} />}</AgentPage>;
}
...
function FilePreview({ path, onClose }: { path: string; onClose: () => void }) {
  ...
  return <div className="preview-overlay" role="dialog" aria-modal="true"><div className="preview-window"><header><strong>{path.split("/").pop()}</strong><button className="icon-button" onClick={onClose} title="关闭" aria-label="关闭预览"><X size={17} /></button></header><div>{content}</div></div></div>;
}
