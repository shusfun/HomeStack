...
function Media() {
  ...
  return <AgentPage title="影视" action={<button className="icon-button" onClick={() => void load()} title="刷新" aria-label="刷新"><RefreshCw size={16} /></button>}>{selected && <section className="player"><video controls autoPlay src={`/api/v1/media/Videos/${encodeURIComponent(selected.item.Id)}/stream.mp4`} onPlay={(event) => void report("Playing", event.currentTarget)} onPause={(event) => void report("Playing/Progress", event.currentTarget)} onTimeUpdate={(event) => progress(event.currentTarget)} onEnded={(event) => void report("Playing/Stopped", event.currentTarget)} /><div><strong>{selected.item.Name}</strong><button onClick={() => setSelected(null)}>关闭</button></div></section>}<div className="media-grid">{items.map((item) => <button key={item.Id} onClick={() => void select(item)}><img src={`/api/v1/media/Items/${encodeURIComponent(item.Id)}/Images/Primary?maxWidth=480&quality=85`} alt="" loading="lazy" /><span><strong>{item.Name}</strong><small>{item.SeriesName || item.ProductionYear || item.Type}</small></span></button>)}</div>{items.length === 0 && !error && <EmptyState icon={<Film size={24} />} label="媒体库为空" />}<Feedback error={error} /></AgentPage>;
}

function Services() {
  const [services, setServices] = useState<ServiceStatus[]>([]); const [error, setError] = useState("");
  const load = useCallback(async () => { setError(""); try { setServices((await api<{ services: ServiceStatus[] }>("/api/v1/services")).services); } catch (reason) { setError(errorMessage(reason)); } }, []);
  useEffect(() => { void load(); }, [load]);
  async function action(id: string, value: string) { setError(""); try { await api<void>(`/api/v1/services/${encodeURIComponent(id)}/actions`, { method: "POST", body: JSON.stringify({ action: value }) }); await load(); } catch (reason) { setError(errorMessage(reason)); } }
...
function Logs() {
  ...
  const load = useCallback(async (after = "") => { setError(""); try { const page = await api<{ entries: LogEntry[]; next_cursor?: string }>(`/api/v1/logs?service=${encodeURIComponent(service)}&limit=200${after ? `&cursor=${encodeURIComponent(after)}` : ""}`); setEntries(after ? (current) => [...current, ...page.entries] : page.entries); setCursor(page.next_cursor || ""); } catch (reason) { setError(errorMessage(reason)); } }, [service]);
  useEffect(() => { void load(); }, [load]);
  return <AgentPage title="日志" action={<select value={service} onChange={(event) => setService(event.target.value)}><option value="homestack-agent">Agent</option><option value="tailscale">Tailscale</option><option value="filebrowser">FileBrowser</option><option value="jellyfin">Jellyfin</option></select>}><div className="log-view">{entries.map((entry) => <div key={entry.cursor}><time>{formatTime(entry.timestamp)}</time><code>{entry.message}</code></div>)}</div>{cursor && <button className="secondary-button" onClick={() => void load(cursor)}>加载后续</button>}<Feedback error={error} /></AgentPage>;
}
...
function AgentUpdates() {
  ...
  const load = useCallback(async () => { setStatus(await api<AgentUpdateStatus>("/api/v1/updates/status")); }, []);
  const check = useCallback(async () => { setError(""); try { setStatus(await api<AgentUpdateStatus>("/api/v1/updates/check", { method: "POST" })); } catch (reason) { setError(errorMessage(reason)); } }, []);
  useEffect(() => { void load().catch((reason) => setError(errorMessage(reason))); }, [load]);
  async function download() {
    setBusy(true); setError("");
    const operation = api<AgentUpdateStatus>("/api/v1/updates/download", { method: "POST" });
