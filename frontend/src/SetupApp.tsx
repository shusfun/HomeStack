...
function ConfigurationStep({ initial, onPrepared, busy, setBusy, error, setError }: StepProps & { initial?: PublicConfiguration; onPrepared: (status: SetupStatus) => void }) {
  ...
  return <section className="setup-panel"><div className="setup-heading"><ServerCog size={22} /><div><h1>VPS 与登录</h1><p>配置公网地址和一种登录方式</p></div></div><form className="setup-form" onSubmit={(event) => void submit(event)}><label>VPS 地址<input value={config.public_host} placeholder="hs.waasabi.cloud" onChange={(event) => setConfig({ ...config, public_host: event.target.value })} required /></label><fieldset><legend>登录方式</legend><div className="segmented-control"><button type="button" aria-pressed={config.provider === "google"} onClick={() => selectProvider("google")}>Google</button><button type="button" aria-pressed={config.provider === "github"} onClick={() => selectProvider("github")}>GitHub</button></div></fieldset><label>OAuth Client ID<input value={config.client_id} onChange={(event) => setConfig({ ...config, client_id: event.target.value })} required /></label><label>OAuth Client Secret<input type="password" autoComplete="new-password" value={config.client_secret} onChange={(event) => setConfig({ ...config, client_secret: event.target.value })} required /></label>{callback && <label>OAuth 回调地址<output key={config.provider}>{callback}</output></label>}<button className="primary-button" disabled={busy || !callback}>{busy ? <LoaderCircle className="spin" size={16} /> : <ServerCog size={16} />}校验并保存</button></form><Feedback error={error} /></section>;
}
