...
func Check(ctx context.Context, spec Spec) protocol.ModuleStatus {
	checkedAt := time.Now().UTC()
	path, err := exec.LookPath(spec.Binary)
	if err != nil {
		return protocol.ModuleStatus{ID: spec.ID, State: "missing", ExpectedVersion: spec.ExpectedVersion, Detail: err.Error(), CheckedAt: checkedAt}
	}
	command := exec.CommandContext(ctx, path, spec.VersionArgs...)
	output, err := command.CombinedOutput()
	if err != nil {
		return protocol.ModuleStatus{ID: spec.ID, State: "error", ExpectedVersion: spec.ExpectedVersion, Detail: commandError(err, output), CheckedAt: checkedAt}
	}
	versionText := strings.TrimSpace(string(output))
	if !spec.VersionPattern.MatchString(versionText) {
		return protocol.ModuleStatus{ID: spec.ID, State: "version_mismatch", Version: versionText, ExpectedVersion: spec.ExpectedVersion, Detail: "检测到的版本与固定版本不一致", CheckedAt: checkedAt}
	}
	return protocol.ModuleStatus{ID: spec.ID, State: "ready", Version: spec.ExpectedVersion, ExpectedVersion: spec.ExpectedVersion, CheckedAt: checkedAt}
}
