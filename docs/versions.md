...
| 组件 | 固定版本 | 用途 |
| --- | --- | --- |
| HomeStack | `0.2.11` | Control、App 与 Node |
...
FileBrowser Quantum 与 Jellyfin 的固定上游资产由 HomeStack Release 镜像；macOS FileBrowser 在发布阶段使用与上游打包版本一致且摘要固定的 UPX 解压，以兼容系统执行策略。签名组件清单固定最终平台资产的架构、大小与 SHA-256；客户端下载不经过 Control VPS。
